$(document).ready(function() {
	
	$("#generate").click(function() {
		Generate_Report();
	});
	
	$("#reset").click(function() {
		Reset_Report();
	});

	$("#i_paytype").blur(function() {
		Valid_Trans_type();
	});
	
	$("#i_sub_code").blur(function() {
		Valid_Sub_Code();
	});
	
	$("#i_fromdate").blur(function() {
		Valid_From_Date();
	});
	
	$("#i_todate").blur(function() {
		Valid_To_Date();
	});

	$("#i_famount").blur(function() {
		Valid_Amount();
	});
	
	$('#view').hide();
	
	Init();
});

function Init()
{
	table = $('#example').DataTable( {
					  "paging":true,
					  "destroy": true,
					  "deferRender": true,
					  "responsive": true,
					  "dom" :
						    "<'row'<'col-sm-4'l><'col-sm-4 text-center'B><'col-sm-4'f>>"+
						    "<'row'<'col-sm-12'tr>>" +
						    "<'row'<'col-sm-6'i><'col-sm-6'p>>",
			          "buttons": [
			                    {
			                        extend: 'excelHtml5',
			                       // title: data.Report_Title,
			                        text:'Excel',
			                      //  filename: data.Report_Name,		           
			                        className: 'btn btn-secondary' 
			                    },
			                    {
			                        extend: 'pdfHtml5',
			                      //  title: data.Report_Title,
			                        text: 'PDF',
			                       // filename: data.Report_Name,		 
			                        className: 'btn btn-secondary',
			                        orientation: 'landscape',
			                        pageSize: 'LEGAL',
			                        customize: function ( doc ) {
			                            doc.content.splice( 0, 0, {
			                                margin: [ 0, 0, 0, 6 ],
			                                alignment: 'left',
			                                image: 'data:image/png;base64,'
			                            } );
			                        }			                    
			                    },
			                    {
			                        extend: 'csvHtml5',
			                       // title: data.Report_Title,
			                        text: 'CSV',
			                      //  filename: data.Report_Name,	
			                        className: 'btn btn-secondary'			                       
			                    },
			                    {
			                        text: 'XML',
			                        className: 'btn btn-secondary xml',	   
			                        action: function ( e, dt, node, config ) 
			                        {	
			                        	//Download_XML_Report();
			                        }
			                    }
			                ],
			          "lengthMenu": [[5, 10, 50, 75, -1], [5, 10, 50, 75, "All"]],
					  "pageLength":5						 
				}); 
} 

var blob_url , blob_file_name; 

function Valid_Trans_type()
{
	if($("#i_paytype").val() == 'Select') 
	{  
		$("#i_paytype_msg").html('Required');  
		$("#i_paytype_msg").show();
		return false;
	}
	else
	{ 
		$("#i_paytype_msg").hide();
		return true;
	}
}

function Valid_Sub_Code()
{
	if($("#i_sub_code").val() == 'Select') 
	{ 
		$("#i_subcode_msg").html('Required');  
		$("#i_subcode_msg").show();
		return false;  
	} 
	else
	{ 
		$("#i_subcode_msg").hide();
		return true;
	}
}

function Valid_From_Date()
{
	if($("#i_fromdate").val() == '')    
	{  
		$("#i_fromdate_msg").html('Required'); 
		$("#i_fromdate_msg").show();
		return false;  
	} 
	else
	{ 
		$("#i_fromdate_msg").hide();
		return true;
	}
}

function Valid_To_Date()
{
	if($("#i_todate").val() == '')       
	{  
		$("#i_todate_msg").html('Required');
		$("#i_todate_msg").show();
		return false;  
	}
	else
	{ 
		$("#i_todate_msg").hide();
		return true;
	}
}

function Valid_Debit_Credit()
{
	if($("#i_dc").val() == 'Select')       
	{ 
		$("#i_dc_msg").html('Required');
		$("#i_dc_msg").show();
		return false;
	}
	else 
	{ 
		$("#i_dc_msg").hide();
		return true;
	}
}

function Valid_Amount()
{
	if($("#i_famount").val() == '')    
	{  
		$("#i_famount_msg").html('Required');  
		$("#i_famount_msg").show();
		return false;
	} 
	else
	{
		$("#i_famount_msg").hide();
		return true;
	}
}

function Generate_Report()
{
	var Trans_type = $("#i_paytype").val() ;  
	var Sub_Code = $("#i_sub_code").val() ;  
	var from_date = $("#i_fromdate").val() ;
	var to_date = $("#i_todate").val(); 
	var Amount = $("#i_famount").val();
	var Reason = $("#i_reason").val(); 
	var Action = $("#i_action").val(); 
	var DC = $("#i_dc").val(); 
	
	var err = 0;
	
	if(!Valid_Trans_type()) 
	{  
		err++; 
	}
	
	if(!Valid_Sub_Code()) 
	{  
		err++; 
	}
	
	if(!Valid_From_Date()) 
	{  
		err++; 
	}
	
	if(!Valid_To_Date()) 
	{  
		err++; 
	}
	
	if(!Valid_Debit_Credit())
	{
		err++; 
	}
	
	if(!Valid_Amount()) 
	{  
		err++; 
	}
	
	if(err != 0) { return; }      
		
	var data = new FormData();
	
	data.append('i_paytype' , Trans_type);
	data.append('i_userid'  , '');
	data.append('i_fromdate', date_format(from_date));
	data.append('i_todate'  , date_format(to_date));
	data.append('i_famount' , Amount);
	data.append('i_tamount' , '');
	data.append('i_reason' , Reason);
	data.append('i_action' , Action);
	data.append('i_dc' , DC);
	

	$.ajax({		 
		url  :  $("#ContextPath").val()+"/Report_Generator/Report/Download",
		type :  'POST',
		data :  data,
		cache : false,
		contentType : false,
		processData : false,
		success: function (data) 
		{ 
			Swal.close();
			
			if(data.Result == 'Success')
			{
				var reports = data.Report;

				reports = eval(reports);
				
				table = $('#example').DataTable( {
					  "aaData": reports,
					  "aoColumns": [
							{ "mData": "S_No"},			
							{ "mData": "Report_Type"},		// Report Type
							{ "mData": "PAYTYPE"},  	    // Transaction Type
							{ "mData": "TRANDATE"},	        // Transaction Date
							{ "mData": "VALUEDATE"},		// Value_Date    
							{ "mData": "TRAN_DB_CR_FLG"},	// Debit/ Credit
							{ "mData": "TRANAMT"},	   	    // Transaction Amount local   //Transaction.addProperty("LOC_CURR", Info.get(i).getLOCALCURR());
							{ "mData": "LOC_CURR"},			// Transaction Amount currency
							{ "mData": "REMARKS1"},			// Remarks
							{ "mData": "ACCOUNTNO"},		// Debit Account
							{ "mData": "BACCOUNTNO"},		// Credit Account
							{ "mData": "SRCBANK"},		    // Source Bank 
							{ "mData": "DSTBANK"}			// Destination Bank 
					  ],
					  "paging":true,
					  "destroy": true,
					  "deferRender": true,
					  "responsive": true,
					  "dom" :
						    "<'row'<'col-sm-4'l><'col-sm-4 text-center'B><'col-sm-4'f>>"+
						    "<'row'<'col-sm-12'tr>>" +
						    "<'row'<'col-sm-6'i><'col-sm-6'p>>",
			          "buttons": [
			                    {
			                        extend: 'excelHtml5',
			                        title: data.Report_Title,
			                        text:'Excel',
			                        filename: data.Report_Name,		           
			                        className: 'btn btn-secondary' 
			                    },
			                    {
			                        extend: 'pdfHtml5',
			                        title: data.Report_Title,
			                        text: 'PDF',
			                        filename: data.Report_Name,		 
			                        className: 'btn btn-secondary',
			                        orientation: 'landscape',
			                        pageSize: 'LEGAL',
			                        customize: function ( doc ) {
			                            doc.content.splice( 0, 0, {
			                                margin: [ 0, 0, 0, 6 ],
			                                alignment: 'left',
			                                image: 'data:image/png;base64,'
			                            } );
			                        }			                    
			                    },
			                    {
			                        extend: 'csvHtml5',
			                        title: data.Report_Title,
			                        text: 'CSV',
			                        filename: data.Report_Name,	
			                        className: 'btn btn-secondary'			                       
			                    },
			                    {
			                        text: 'XML',
			                        className: 'btn btn-secondary xml',	   
			                        action: function ( e, dt, node, config ) 
			                        {	
			                        	Download_XML_Report();
			                        }
			                    }
			                ],
			          "lengthMenu": [[5, 10, 50, 75, -1], [5, 10, 50, 75, "All"]],
					  "pageLength":5						 
				}); 
				
				var xmltext = data.Blob_String;
				
				var bb = new Blob([xmltext], {type: 'text/plain'});

				//$(".xml").attr("href", window.URL.createObjectURL(bb));
				//$(".xml").attr("download", data.file_name);	
				
				//$("#download").attr("href", window.URL.createObjectURL(bb));
				//$("#download").attr("download", data.file_name);
				
				blob_url = window.URL.createObjectURL(bb);
				
				blob_file_name = data.file_name;
				
				$('#view').show();
				
				$('#Modal2').modal('show'); 	
				
			/*	var xmltext = data.Blob_String;
			
				var bb = new Blob([xmltext], {type: 'text/plain'});

				$("#download").attr("href", window.URL.createObjectURL(bb));
				$("#download").attr("download", data.file_name);
				
				$('#modal').modal('show'); 		*/	
			}
			else
			{
				Sweetalert("warning", "", data.Message);
			}
		},
		beforeSend: function( xhr )
		{
			$('#Modal2').modal('hide');
			
			$('#view').hide();
			
			blob_url = ""; blob_file_name="";
			
			Sweetalert("load", "", "Generating Report...")
        },
	    error: function (jqXHR, textStatus, errorThrown) 
	    { 
	    	
	    }
   });
}

function Download_XML_Report() 
{
	  var link = document.createElement("a");
	  link.download = blob_file_name;
	  link.href = blob_url;
	  document.body.appendChild(link);
	  link.click();
	  document.body.removeChild(link);
	  delete link;
}

function Reset_Report()
{
	$("#i_paytype").val('Select') ;  
	$("#i_sub_code").val('Select') ;  
	$("#i_fromdate").val('') ;
	$("#i_todate").val(''); 
	$("#i_famount").val('');
	$("#i_reason").val(''); 
	$("#i_action").val(''); 
	$("#i_dc").val('Select'); 
	
	$("#view").hide();
}

function date_format(val)
{
    var dt = new Date(val); 

    var dd = dt.getDate(); 
    var mm = dt.getMonth() + 1; 

    var yyyy = dt.getFullYear(); 
    if (dd < 10) { 
        dd = '0' + dd; 
    } 
    if (mm < 10) { 
        mm = '0' + mm; 
    } 
    
    var condate = dd + '-' + mm + '-' + yyyy; 
    
    return condate;

}

function Sweetalert(Type, Title, Info)
{
	if(Type == "success")
	{
		Swal.fire({
			  icon: 'success',
			  title: Title ,
			  text: Info ,			 
			  timer: 2000
		});
	}
	else if(Type == "success_load_Current")
	{
		Swal.fire({
			  icon: 'success',
			  title: Title ,
			  text:  Info,
			  timer: 2000
		}).then(function (result) {
			  if (true)
			  {							  
				  location.reload();
			  }
		});		
	}
	else if(Type == "error")
	{
		Swal.fire({
			  icon: 'error',
			  title: Title ,
			  text:  Info ,		 
			  timer: 2000
		});
	}
	else if(Type == "warning")
	{
		Swal.fire({
			  icon: 'warning',
			  title: Title ,
			  text:  Info, 	 
			  timer: 2000
		});
	}
	else if(Type == "load")
	{
		Swal.fire({
			  title: Title,
			  html:  Info,
			  timerProgressBar: true,
			  allowOutsideClick: false,
			  onBeforeOpen: () => {
			    Swal.showLoading()
			  },
			  onClose: () => {
			  }
		});	
	}	
}