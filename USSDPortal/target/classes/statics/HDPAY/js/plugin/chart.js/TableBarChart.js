jQuery.fn.tableBarChart = function(targetDiv, caption, reverseGroup) {
	...
	function tablebargetHeaderNames(isColumn) {
	...
	function tablebargetDataByColumn() {
		var dataMatrix = [];   
		var numCols = source.find("tbody tr:eq(0) td").length; 	
	...
	function tablebargetAxisHTML(sourceArray, cssClass) {
	...
	function tablebargetYAxisArray(stepCount) {
	...
	function tablebaranimateBar(index) { 
	...
	function tablebargetLegendHTML() {
	...
	function tablebargetBarChartHTML() {
	...
	function tablebarlayout() {  
		...
		for (var i = 0; i < legends.length; i++) {
			...
			tablebaranimateBar(i);
		}
	}
	
	function render() {  
		target.append('<div class="caption">' + tablebargetCaption() + '</div>');
		var graphContainer = $('<div class="graph"></div>');
		graphContainer.append(tablebargetAxisHTML(groupTitles, 'x-axis'));
		graphContainer.append(tablebargetAxisHTML(tablebargetYAxisArray(groupTitles.length + 1), 'y-axis'));
		graphContainer.append(tablebargetBarChartHTML());
		target.append(graphContainer);
		target.append(tablebargetLegendHTML());
		tablebarlayout();
	}
	
	function tablebarinitialize() { 
		groupTitles = tablebargetHeaderNames(reverseGroup ? false : true); 
		legends = tablebargetHeaderNames(reverseGroup ? true : false);				
		tableData = reverseGroup ? tablebargetDataByRow() : tablebargetDataByColumn(); 
		render();
	}
